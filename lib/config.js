...
function normalizeUpstream(u, i) {
  ...
  // Sticky is the default (cache warmth / fewer cache misses). Set `"sticky": false`
  // on an upstream to opt back into utilization-based balancing.
  const sticky = u.sticky !== false;
  return { name, baseUrl, models, keys, adapter, rateLimitHeaders, unauthorizedPolicy, sticky };
}
