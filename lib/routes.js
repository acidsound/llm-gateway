...
export class RouteStore {
  ...
  get(model) {
    // Exact match first
    if (model in this.routes) return this.routes[model];
    // Then glob patterns (longest/most specific first)
    for (const { regex } of this.patterns) {
      if (regex.test(model)) return this.routes[this.patterns.find((p) => p.regex === regex).pattern];
    }
    return undefined;
  }
  ...
  resolve(model) {
    ...
    for (const { pattern, regex } of this.patterns) {
      if (regex.test(model)) return this.routes[pattern];
    }
    return undefined;
  }
