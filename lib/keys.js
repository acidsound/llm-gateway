...
export class KeyPool {
  ...
  select(exclude = [], model = null) {
    ...
    // as it stays healthy.  We only fall through to balancing (and thus rotate)
    // when the preferred key is disabled, already tried in this request, or
    // cooling down after a real upstream failure (429 / 5xx / network error).
    // A key that responds 200 is fine regardless of local utilization estimates;
    // the RPM/TPM window is a local heuristic, not a real provider limit, so
    // rotating purely on utilization churns healthy keys and breaks cache warmth.
    if (this.sticky) {
