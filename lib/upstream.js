...
export class UpstreamProxy {
  ...
  async attemptUpstream(upstream, model, ctx) {
    ...
    while (true) {
      ...
      if (result.ok) {
        ...
        if (emptyFailure) {
          await this.cancelBody(result.upstream);
          this.handleFailure(pool, result.key, model, { status: 502, kind: 'network', message: 'upstream returned empty response (native network_error)' });
  ...
  async peekStreamForNetworkError(upstream) {
    ...
    if (!text.includes('"native_finish_reason":"network_error"')) return false;
    // Extract content / tool_calls / reasoning from all data chunks.
    const hasPayload = [...text.matchAll(/data: (\{.*\})/g)].some((m) => {
      try {
        const delta = JSON.parse(m[1])?.choices?.[0]?.delta || {};
        return delta.content || delta.tool_calls?.length || delta.reasoning;
      } catch {
        return false;
      }
    });
  ...
  async pipeToClient(res, upstream, upstreamConf, key, isStream) {
    ...
    if (isStream) {
      ...
      res.flushHeaders();
      // Buffered replay (peeked stream): write the stored bytes verbatim.
